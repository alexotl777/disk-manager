...
async def get_disks(
    ...
    def collect(node, parent_name=None):
        ...
        dev_path = f"/dev/{name}"
        devices.append({"name": name, "device": dev_path, "size": size, "mountpoint": mount})
    ...
    return JSONResponse({"devices": devices, "allowed_prefixes": disks_service.allowed_device_prefixes})
...
async def make_unmount(
    ...
    if not disks_service.is_device_allowed(unmount.device):
        raise HTTPException(status_code=400, detail=f"Device {unmount.device} not allowed by server policy.")
    # umount
    res = disks_service.run_cmd(f"sudo umount {shlex.quote(unmount.device)}", check=False)
...
async def make_mount(
    ...
    if not disks_service.is_device_allowed(mount.device):
        raise HTTPException(status_code=400, detail=f"Device {mount.device} not allowed by server policy.")

    try:
        os.makedirs(os.path.expanduser(mount.mount_point), exist_ok=True)
    except Exception as e:
        logger.error(f"error for {mount.mount_point} - {e}")
        raise HTTPException(status_code=400, detail=f"Failed to create mount point: {e}")
    # mount
    res = disks_service.run_cmd(f"sudo mount {shlex.quote(mount.device)} {shlex.quote(os.path.expanduser(mount.mount_point))}", check=True)
    return JSONResponse(res)
...
async def make_format(
    ...
    if not disks_service.is_device_allowed(format_disk.device) and not config.enable_dangerous_ops:
    ...
    check = disks_service.run_cmd(f"lsblk -n -o MOUNTPOINT {shlex.quote(format_disk.device)}", check=False)
